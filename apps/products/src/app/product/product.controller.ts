...
export class ProductController {
  ...
  @MessagePattern(EVENT_MESSAGES.PRODUCT.CREATE)
  create(@Payload() createProductDto: Core.DTO.Products.CreateProductDto) {
    return this.productService.create(createProductDto);
  }

  @MessagePattern(EVENT_MESSAGES.PRODUCT.FIND_ALL)
  findAll(@Payload() paginationDTO: PaginationDTO) {
    return this.productService.findAll(paginationDTO);
  }

  @MessagePattern(EVENT_MESSAGES.PRODUCT.FIND_ONE)
  findOne(@Payload('id', ParseIntPipe) id: number) {
    return this.productService.findOne(id);
  }

  @MessagePattern(EVENT_MESSAGES.PRODUCT.UPDATE)
  ...
  @MessagePattern(EVENT_MESSAGES.PRODUCT.DELETE)
  remove(@Payload('id') id: string) {
    return this.productService.remove(+id);
  }

  @MessagePattern(EVENT_MESSAGES.PRODUCT.VALIDATE)
